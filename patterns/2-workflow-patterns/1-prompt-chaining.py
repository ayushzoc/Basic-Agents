...
class EventDetails(BaseModel):
    ...
    date: str = Field(
        dsecription="Date and time of the event. Use ISO 8601 to format this value."
    )
...
class EventConfirmation(BaseModel):
    ...
    confirmation_message: str = Field(
        descrption="Natural language confirmation message"
    )
...
def extract_event_info(user_input: str) -> EventExtraction:
    ...
    logger.info(
        f"Extraction complete - Is calendar event: {result.is_calendar_event}, Confidence: {result.confidence_score}"
    )
